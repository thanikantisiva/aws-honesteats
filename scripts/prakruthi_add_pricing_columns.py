#!/usr/bin/env python3
"""
Add optional Zomato column + pricing columns to Prakruthi_Millet_Foods_Menu.xlsx

Zomato commission 25% -> R_z = Z * 0.75
Our commission 25% -> restaurant P * 0.75, platform P * 0.25

P = (0.40 * Z + 0.60 * R_z) / 0.85, rounded to nearest ₹1

No Zomato (blank / invalid): P = round(D * 1.35), Our Hike on Dine-in % = 35.

If the sheet has only 4 columns (no Zomato yet), inserts column E
'Zomato Price (₹)' for manual entry.
"""
...
XLSX = "/Users/user/Downloads/Prakruthi_Millet_Foods_Menu.xlsx"

COL_D = 4  # Dine-in (Price (₹))
COL_Z = 5  # Zomato Price (₹) — inserted if missing

REST_ZOMATO = 0.75
REST_US = 0.75
OUR_COMM = 0.25
FIXED_HIKE_NO_Z = 0.35
...
def main() -> None:
    ...
    if ws.max_column == 4:
        ws.insert_cols(5, 1)
        zc = ws.cell(row=1, column=5, value="Zomato Price (₹)")
        zc.font = Font(bold=True)
        zc.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
