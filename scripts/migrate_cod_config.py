#!/usr/bin/env python3
"""
Migrate the global COD config (codConfig: disableCod / minAmount / maxAmount) out
of the global config row (CONFIG#GLOBAL) into the dedicated row (CONFIG#COD).

Uses the HTTP API (no AWS creds needed):
  GET  /api/v1/globalconfig          -> read codConfig
  POST /api/v1/cod-config            -> write it to the dedicated row
  POST /api/v1/globalconfig (--strip)-> rewrite global config without codConfig

Target environment: --env dev|prod (default prod), or --api-url <host>, or
$HONESTEATS_API_URL. Precedence: --api-url > --env > env var > prod.

Usage:
  python3 scripts/migrate_cod_config.py --env dev                 # dry run (dev)
  python3 scripts/migrate_cod_config.py --env dev --apply         # copy into CONFIG#COD
  python3 scripts/migrate_cod_config.py --env prod --apply --strip # also remove from global
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Migrate COD config to its own row")
    ap.add_argument("--apply", action="store_true", help="perform writes (default: dry run)")
    ap.add_argument("--strip", action="store_true", help="also remove codConfig from CONFIG#GLOBAL")
    ...
    cod_config = global_config.get(COD_KEY)
    if not isinstance(cod_config, dict) or not cod_config:
        print("Nothing to migrate — no codConfig found in global config.")
        return 0

    print("COD config found in global row:")
    print(json.dumps(cod_config, indent=2, ensure_ascii=False))
    print(f"\nPlan:\n  → POST {base}/api/v1/cod-config       (write codConfig to CONFIG#COD)")
    if args.strip:
        print(f"  → POST {base}/api/v1/globalconfig     (rewrite global WITHOUT codConfig)")
    ...
    w = s.post(f"{base}/api/v1/cod-config", json={COD_KEY: cod_config}, timeout=60)
    if w.status_code != 200:
        print(f"✗ POST /cod-config failed: HTTP {w.status_code} {w.text[:200]}", file=sys.stderr)
        return 1
    print("✓ Wrote CONFIG#COD")

    # 3) Optionally strip the key from the global row.
    if args.strip:
        cleaned = {k: v for k, v in global_config.items() if k != COD_KEY}
        ...
        if g.status_code not in (200, 201):
            ...
            return 1
        print("✓ Removed codConfig from CONFIG#GLOBAL")
