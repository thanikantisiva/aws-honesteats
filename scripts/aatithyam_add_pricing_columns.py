#!/usr/bin/env python3
"""
Add pricing columns to /Users/user/Downloads/Aatithyam_Zomato_vs_DineIn.xlsx

Zomato commission 42% -> R_z = Z * 0.58
Our commission 22% -> P * 0.78 to restaurant, P * 0.22 platform

P = (0.40 * Z + 0.60 * R_z) / 0.85, rounded to nearest ₹1

If Zomato (₹) is '-': P = round(D * 1.28), Our Hike on Dine-in % = 28 (fixed).
"""
...
XLSX = "/Users/user/Downloads/Aatithyam_Zomato_vs_DineIn.xlsx"
...
def main() -> None:
    ...
    for r in range(2, ws.max_row + 1):
        ...
        z_val = ws.cell(row=r, column=5).value
        if d_val is None:
            continue
        D = float(d_val)
        if D <= 0:
