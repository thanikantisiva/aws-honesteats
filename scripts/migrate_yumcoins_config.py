#!/usr/bin/env python3
"""
Migrate YumCoins config (walletConfig / referralConfig / orderCashbackConfig) out
of the global config row (CONFIG#GLOBAL) into the dedicated row (CONFIG#YUMCOINS).

Uses the HTTP API (no AWS creds needed):
  GET  /api/v1/globalconfig          -> read the three coin objects
  POST /api/v1/yumcoins-config       -> write them to the dedicated row
  POST /api/v1/globalconfig (--strip)-> rewrite global config without the three keys

Target environment: --env dev|prod (default prod), or --api-url <host>, or
$HONESTEATS_API_URL. Precedence: --api-url > --env > env var > prod.

Usage:
  python3 scripts/migrate_yumcoins_config.py --env dev                 # dry run (dev)
  python3 scripts/migrate_yumcoins_config.py --env dev --apply         # copy into CONFIG#YUMCOINS
  python3 scripts/migrate_yumcoins_config.py --env prod --apply --strip # also remove from global
"""
...
KEYS = ("walletConfig", "referralConfig", "orderCashbackConfig")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Migrate YumCoins config to its own row")
    ...
    coin_config = {k: global_config[k] for k in KEYS if k in global_config}
    if not coin_config:
        print("Nothing to migrate — no walletConfig/referralConfig/orderCashbackConfig in global config.")
        return 0

    print("YumCoins config found in global row:")
    print(json.dumps(coin_config, indent=2, ensure_ascii=False))
    print(f"\nPlan:\n  → POST {base}/api/v1/yumcoins-config  (write the 3 objects)")
    if args.strip:
        print(f"  → POST {base}/api/v1/globalconfig     (rewrite global WITHOUT the 3 keys)")
    ...
    w = s.post(f"{base}/api/v1/yumcoins-config", json=coin_config, timeout=60)
    if w.status_code != 200:
        print(f"✗ POST /yumcoins-config failed: HTTP {w.status_code} {w.text[:200]}", file=sys.stderr)
        return 1
    print("✓ Wrote CONFIG#YUMCOINS")
    ...
    if args.strip:
        ...
        if g.status_code not in (200, 201):
            ...
            return 1
        print("✓ Removed the 3 keys from CONFIG#GLOBAL")
