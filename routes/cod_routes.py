...
lives in the global config row and is set via POST /api/v1/globalconfig.
"""
...
def _fetch_cod_config() -> dict:
    """Read the `codConfig` block from the global config row (empty dict if absent)."""
    ...
    if not isinstance(config_payload, dict):
        return {}
    cod_config = config_payload.get("codConfig")
    return cod_config if isinstance(cod_config, dict) else {}


def _decide_cod(disable_cod: bool, force_cod: bool, bill: float, cod_config: dict) -> dict:
...
def register_cod_routes(app):
    ...
    def check_cod_eligibility():
        ...
        try:
            ...
            cod_config = _fetch_cod_config()
            decision = _decide_cod(disable_cod, force_cod, bill, cod_config)

            logger.info(
                f"COD eligibility: bill={bill} disableCod={disable_cod} "
                f"forceCod={force_cod} -> {decision['reason']}"
            )
